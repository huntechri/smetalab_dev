...
2. Нажмите **New ruleset → Import a ruleset** и загрузите `.github/rulesets/main-protection.json`.  
   Либо создайте вручную: **New branch ruleset**, target `main`/`master`, добавьте правило **Require status checks → All Checks Passed**.
3. Сохраните.

После применения любого из способов GitHub блокирует кнопку "Merge" на любом PR, где хотя бы одна из проверок (lint, type-check, test) завершилась с ошибкой.
