...
### Настройка branch protection rule (для администратора репозитория)

Правило защиты применяется через встроенный workflow **`.github/workflows/setup-branch-protection.yml`**:
...
3. Workflow прочитает текущие настройки защиты, добавит **`All Checks Passed`** в список обязательных проверок и применит обновление, сохранив все остальные настройки (проверки кода, ограничения, enforce_admins и т.д.) без изменений.

Если предпочитаете ручную настройку:

1. Перейдите в **Settings → Branches** репозитория.
2. Нажмите **Add branch protection rule** (или отредактируйте существующее для `main`/`master`).
3. В поле **Branch name pattern** введите `main` (или `master`).
4. Включите **Require status checks to pass before merging**.
5. В поиске статус-чеков найдите и добавьте **`All Checks Passed`** (это job `ci` из `.github/workflows/ci.yml`).
6. Включите **Require branches to be up to date before merging**.
7. Сохраните правило.

После этого GitHub блокирует кнопку "Merge" на любом PR, где хотя бы одна из проверок (lint, type-check, test) завершилась с ошибкой.
