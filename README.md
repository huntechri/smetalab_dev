...
- **AI-поиск материалов:** Используется двухфазный пайплайн (кандидаты по FTS/trgm + векторный top-K, затем re-rank), чтобы задействовать индексы и ускорить поиск на больших справочниках.
