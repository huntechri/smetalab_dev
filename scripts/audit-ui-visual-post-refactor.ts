...
const RULES: PatternRule[] = [
  ...
  },
  {
    ...
    getSeverity: severityForTypography,
  },
  {
    ...
    getSeverity: (filePath, surface, _token, ownership) =>
      ownership === 'primitive-contract' || ownership === 'feature-family-contract'
        ? 'low'
        : isBusinessRuntimeSurface(surface, filePath)
          ? downgradeForMarketingAuth('medium', filePath)
          : 'low',
  },
  {
...
function severityForTypography(filePath: string, surface: Surface, _token: string, ownership: Ownership): Severity {
  if (ownership === 'primitive-contract' || ownership === 'feature-family-contract') return 'low'
  if (isBusinessRuntimeSurface(surface, filePath)) return downgradeForMarketingAuth('medium', filePath)
  return 'low'
}

function reasonSuffix(ownership: Ownership): string {
  ...
  for (const rule of RULES) {
    for (const match of line.matchAll(rule.pattern)) {
      ...
      addFinding(findings, seen, {
        ...
        severity,
        token,
        reason: `${rule.reason}${reasonSuffix(ownership)}`,
        surface,
        ownership,
      })
    }
...
function countScannedRoots(files: string[]): Record<string, number> {
  ...
  for (const filePath of files) {
    const rootName = getRootName(toPosix(path.relative(ROOT, filePath)))
...
function validateTarget(target: TargetSurface, findings: Finding[]): TargetValidation {
  ...
  const targetFindingsAreAcceptedSharedResiduals =
    targetFindings.length > 0 &&
    targetFindings.every((finding) => sharedResidualOwners.includes(finding.filePath) && isAcceptedSharedContract(finding.filePath))
  ...
  if (targetHighCritical.length > 0) {
    ...
    status = 'REVIEW'
  } else if (targetFindingsAreAcceptedSharedResiduals) {
...
function writeReports(report: AuditReport): void {
  ...
  const highPriority = sortedFindings.filter(isHighCritical)
...
function run(): void {
  ...
  const reportEnabled = strict || process.argv.includes('--report')
  ...
  if (reportEnabled) writeReports(report)

  const businessRuntimeHighPriority = findings.filter((finding) => finding.ownership === 'business-runtime-drift' && isHighCritical(finding))
  if (strict && businessRuntimeHighPriority.length > 0) {
    console.error(`UI visual audit failed: ${businessRuntimeHighPriority.length} business-runtime High/Critical finding(s).`)
    process.exitCode = 1
    return
  }

  console.log(
    `UI visual audit complete: ${report.totalFindings} finding(s), ${businessRuntimeHighPriority.length} business-runtime High/Critical finding(s).`,
  )
}
