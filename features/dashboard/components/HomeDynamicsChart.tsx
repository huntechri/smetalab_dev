...
type HomeSeriesKey = keyof typeof chartConfig;

const homeSeriesOptions: Array<{ key: HomeSeriesKey; label: string }> = [
...
const defaultVisibleSeries: Record<HomeSeriesKey, boolean> = {
    receiptsFact: true,
    executionPlan: true,
    executionFact: true,
...
export function HomeDynamicsChart({ data }: HomeDynamicsChartProps) {
    ...
    const [visibleSeries, setVisibleSeries] = React.useState<Record<HomeSeriesKey, boolean>>(defaultVisibleSeries);
    ...
    const hasActivity = React.useMemo(
        () => {
            ...
            return chartData.some((point) =>
                homeSeriesOptions.some(({ key }) => visibleSeries[key] && Number(point[key] ?? 0) !== 0),
            );
        },
        [mode, flowTimeline, levelTimeline, chartData, visibleSeries],
    );

    const setSeriesVisibility = React.useCallback((key: HomeSeriesKey, checked: boolean) => {
                    ...
                    >
                        <ComposedChart data={chartData}>
                            <defs>
                                ...
                                <linearGradient id="homeFillProcurement" x1="0" y1="0" x2="0" y2="1">
