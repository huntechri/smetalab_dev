...
export interface EstimatesRepository {
    ...
    addWork(estimateId: string, payload?: Partial<Pick<EstimateRow, 'name' | 'unit' | 'qty' | 'price' | 'expense'>>): Promise<EstimateRow>;
    addMaterial(
        estimateId: string,
        parentWorkId: string,
        payload?: Partial<Pick<EstimateRow, 'name' | 'unit' | 'qty' | 'price' | 'expense'>>,
    ): Promise<EstimateRow>;
}
