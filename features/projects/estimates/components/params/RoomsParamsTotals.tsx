...
function TotalLine({ label, totals, isGrandTotal = false }: { label: string; totals: Totals; isGrandTotal?: boolean }) {
    return (
        <div className={`flex flex-row flex-wrap items-center gap-x-4 gap-y-1 ${primitiveCompactTableCellClassName} ${isGrandTotal ? "mt-2 pt-2 border-t border-border/50 " + primitiveVisualTypographyClassNames.compactLabel : 'text-muted-foreground'}`}>
