...
function TotalLine({ label, totals, isGrandTotal = false }: { label: string; totals: Totals; isGrandTotal?: boolean }) {
    return (
        <div className={`flex flex-row flex-wrap items-center gap-x-4 gap-y-1 text-[0.5625rem] sm:text-[0.6875rem] ${isGrandTotal ? 'mt-2 pt-2 border-t border-border/50 font-bold' : 'py-1 text-muted-foreground'}`}>
...
export function RoomsParamsTotals({ rows, grandTotals }: { rows: EstimateRoomParamDraft[]; grandTotals: Totals }) {
    return (
        <div className="mt-4">
            <div className="rounded-md border border-border bg-muted/30 text-card-foreground shadow-none p-3 sm:p-4">
