...
function TotalLine({ label, totals, isGrandTotal = false }: { label: string; totals: Totals; isGrandTotal?: boolean }) {
    return (
        <div className={`flex flex-row flex-wrap items-center gap-x-4 gap-y-1 text-xs sm:text-xs ${isGrandTotal ? 'mt-2 pt-2 border-t border-border/50 font-bold' : 'py-1 text-muted-foreground'}`}>
    ...
    return (
        <Card className="bg-muted/30 border-none shadow-none mt-4">
            <CardContent className="p-3 space-y-0.5">
                <div className="font-bold text-xs sm:text-xs mb-2 uppercase tracking-wider text-muted-foreground">Итого:</div>
