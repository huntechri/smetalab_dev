...
export type EstimateColumnActions = {
    ...
    sectionTotalsById: Map<string, number>;
};
...
export const getEstimateColumns = (actions: EstimateColumnActions): ColumnDef<VisibleEstimateRow>[] => [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            const sectionTotal = row.original.kind === 'section' ? (actions.sectionTotalsById.get(row.original.id) ?? 0) : row.original.sum;
            return (
                <div className={`text-right tabular-nums pr-6 text-xs md:text-sm ${row.original.kind === 'section' ? 'font-semibold' : row.original.kind === 'material' ? "italic text-muted-foreground" : "font-medium text-primary/90"}`}>
                    <MoneyCell value={sectionTotal} />
                </div>
            );
        }
    },
